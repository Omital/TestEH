...
using TestEH.Migrations.SeedData;
...
namespace TestEH.Tests
{
    public abstract class TestEHTestBase : AbpIntegratedTestBase<TestEHTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected TestEHTestBase()
        {
        ...
        protected void UsingDbContext(Action<TestEHDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TestEHDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TestEHDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TestEHDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TestEHDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TestEHDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TestEHDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TestEHDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TestEHDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TestEHDbContext, Task<T>> func)
        {
            T result;
